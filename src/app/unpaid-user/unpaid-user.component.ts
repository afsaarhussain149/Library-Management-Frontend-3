...
@Component({
  ...
  standalone: false,
  templateUrl: './unpaid-user.component.html',
  styleUrl: './unpaid-user.component.css'
})
export class UnpaidUserComponent {
  regpassword: any;
  confirmPassword: any;
  constructor(
...
checkUnpaidUser() {
    ...
    this.http.post(`${this.api}/api/auth/unpaid-user`, {
      phoneNumber: this.unpaidPhone
    }).subscribe({

      next: (res: any) => {
        this.loading = false;

        if (res.paymentStatus === 'not-paid') {
...
sessionStorage.setItem('takeuserdetails',JSON.stringify(res.user))
      ...
      error: (err) => {
        this.loading = false;
        this.notificationsService.error('Error', err.error?.msg || 'User not found');
      }
